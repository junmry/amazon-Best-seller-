...
### 分层标准
| 层级 | 排名区间 | 搜索量映射 | 匹配方式 | 预算占比 |
|------|---------|-----------|---------|---------|
| L1 锚定层 | 1-1,000 | ≥5,000 | 精准匹配+高竞价 | 40% |
| L2 主战场 | 1,001-30,000 | 500-4,999 | 词组匹配分时段加价 | 35% |
| L3 验证区 | 30,001-150,000 | 100-499 | 广泛匹配低竞价养数据 | 20% |
| L4 长尾区 | 150,001-500,000 | 10-99 | 广泛匹配捡漏 | 4% |
| L5 探索区 | 500,001+ | <10 | 自动广告跑词 | 1% |

### 已处理8个品类
**有排名数据（直接分层）:** 吧凳(4394词)、户外家具(5488词)、桌布(9161词)、火坑(853词)、玄关桌(6803词)
**无排名按搜索量映射:** 沙发(6851词)、椅子(14257词)、人体工学椅(15653词)
**未处理:** 餐桌套装（只有词频无搜索量/排名）、躺椅（已有分层作为参考标准）

### 脚本位置
`keyword_library/_广告词库/generate_l_level_flow.py` — 可复用，后续新增品类直接运行即可
